package com.prohitman.unsortedcannibals.common.worldgen.structures;

import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import com.prohitman.unsortedcannibals.core.init.ModStructures;
...
public class FallTrapStructure extends Structure {
    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<FallTrapStructure> CODEC = RecordCodecBuilder.<FallTrapStructure>mapCodec(instance ->
            instance.group(FallTrapStructure.settingsCodec(instance),
            ...
            ).apply(instance, FallTrapStructure::new)).codec();
    ...
    public FallTrapStructure(Structure.StructureSettings config,
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        if (!FallTrapStructure.extraSpawningChecks(context)) {
        ...
        int startHeightY = context.chunkGenerator().getFirstFreeHeight(blockPos.getX(), blockPos.getZ(), Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, context.heightAccessor(), context.randomState());

        // Check the heights around the spawn position to ensure flatness
        int radius = 5; // Distance to check around the spawn position
    ...
    public StructureType<?> type() {
        return ModStructures.FALL_TRAP_STRUCTURE.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}
